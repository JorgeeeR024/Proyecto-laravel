    ...
    <div class="w-full max-w-md">
        <div class="bg-white rounded-xl shadow-2xl overflow-hidden">
            <div class="bg-blue-600 py-5 px-6 text-center">
                <h2 class="text-xl font-bold text-white">Editar Grado</h2>
            </div>
            <div class="p-6">
                <form action="{{ route('admin.grades.update', $grade->id_grade) }}" method="POST">
                    @csrf
                    @method('PUT')

                    <div class="mb-5">
                        <label for="name" class="block text-sm font-medium text-gray-700 mb-2">Nombre del grado</label>
                        <input type="text" name="name" value="{{ $grade->name }}" required
                    ...
                    <div class="flex justify-between mt-8">
                        ...
                        <button type="submit" class="px-5 py-2.5 bg-green-500 hover:bg-green-600 text-white rounded-lg">Actualizar</button>
                    </div>
                </form>
            </div>
        </div>
